...
float xMin      = 0;
float xMax      = 160;
string plotName = "dilepton Mass"; // don't put special symbols and spaces here
string ylabel   = "events / 1 GeV";
string xlabel   = "Mass (l^{+} , l^{-}) GeV";
...
{    
	...
	for(Long64_t j = 0 ; j<nEvents ; ++j)
	{
	    events[i]->GetEntry(j); 
            ...
            if(triggerIsoMu24 == 1 && NMuon >= 1 && NElectron >= 1)
            if(sqrt(Muon_Px[0]*Muon_Px[0] + Muon_Py[0]*Muon_Py[0]) > 25 && Muon_Charge[0]+Electron_Charge[0] == 0) 
            {
                selection = true; 
                int NBJet = 0;
                ...
                float dilepP2 = (Muon_Px[0]+Electron_Px[0])*(Muon_Px[0]+Electron_Px[0]) + (Muon_Py[0]+Electron_Py[0])*(Muon_Py[0]+Electron_Py[0]) + (Muon_Pz[0]+Electron_Pz[0])*(Muon_Pz[0]+Electron_Pz[0]);
                float dilepE2 = (Muon_E[0]+Electron_E[0])*(Muon_E[0]+Electron_E[0]);
                float mass = sqrt(dilepE2 - dilepP2); 
                var       = mass;
