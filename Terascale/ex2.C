...
float xMin      = 0;
float xMax      = 160;
string plotName = "dimuon Mass"; // don't put special symbols and spaces here
string ylabel   = "events / 5 GeV";
...
{    
	...
	for(Long64_t j = 0 ; j<nEvents ; ++j)
	{
	    events[i]->GetEntry(j); 
            ...
            if(triggerIsoMu24 == 1 && NMuon >= 2)
            if(sqrt((Muon_Px[0]+Muon_Px[1])*(Muon_Px[0]+Muon_Px[1]) + (Muon_Py[0]+Muon_Py[1])*(Muon_Py[0]+Muon_Py[1])) > 25)
            {
                selection = true; 
                int NBJet = 0;
                ...
                float dimuP2 = (Muon_Px[0]+Muon_Px[1])*(Muon_Px[0]+Muon_Px[1]) + (Muon_Py[0]+Muon_Py[1])*(Muon_Py[0]+Muon_Py[1]) + (Muon_Pz[0]+Muon_Pz[1])*(Muon_Pz[0]+Muon_Pz[1]);
                float dimuE2 = (Muon_E[0]+Muon_E[1])*(Muon_E[0]+Muon_E[1]);
                float mass = sqrt(dimuE2 - dimuP2); 
                var       = mass;
