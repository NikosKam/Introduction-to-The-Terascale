...
float xMin      = 30;
float xMax      = 200;
string plotName = "Dijet and Leading bjet Mass"; // don't put special symbols and spaces here
string ylabel   = "events / 10 GeV";
string xlabel   = "Dijet and Leading bjet Mass M_{qqb1} GeV";
...
{    
	...
	for(Long64_t j = 0 ; j<nEvents ; ++j)
	{
	    events[i]->GetEntry(j); 
            ...
            if(triggerIsoMu24 == 1 && NMuon == 1 && NJet  >= 4)
            if(sqrt(Muon_Px[0]*Muon_Px[0] + Muon_Py[0]*Muon_Py[0]) > 25)
            {
                ...
                for (int JetIndex = 0; JetIndex < NJet; JetIndex++)
                {
                    if(Jet_btag[JetIndex] >= 1.74)NBJet += 1;
                        //BJet_Px[0] =Jet_Px[JetIndex];
                        //BJet_Py[0] =Jet_Py[JetIndex];
                        //BJet_Pz[0] =Jet_Pz[JetIndex];
                        //BJet_E[0] =Jet_E[JetIndex];
                   
                }

                if(NBJet>=2){ selection = true;
                ...
                for (int JetIndex = 0; JetIndex < NJet; JetIndex++)
                {
                    if(Jet_btag[JetIndex] >= 1.74) { //NBJet += 1;
                            ...
                            for (int n=0; n < NJet && n!=JetIndex; n++)
                            {
                                float dijetP2 = (Jet_Px[n]*Jet_Px[n] + Jet_Py[n]*Jet_Py[n] + Jet_Pz[n]*Jet_Pz[n]);
                                float dijetE2 = (Jet_E[n]*Jet_E[n]);
                                mass1[0] = sqrt(dijetE2 - dijetP2);
                                lq = n;
                            }
                            
                        }
                }
                //float dijetP2 = (Jet_Px[0]+Jet_Px[1])*(Jet_Px[0]+Jet_Px[1]) + (Jet_Py[0]+Jet_Py[1])*(Jet_Py[0]+Jet_Py[1]) + (Jet_Pz[0]+Jet_Pz[1])*(Jet_Pz[0]+Jet_Pz[1]);
                //float dijetE2 = (Jet_E[0]+Jet_E[1])*(Jet_E[0]+Jet_E[1]);
                //float mass = sqrt(dijetE2 - dijetP2);

                float bjetP2 = (BJet_Px[0]*BJet_Px[0] + BJet_Py[0]*BJet_Py[0] + BJet_Pz[0]*BJet_Pz[0]);
                float bjetE2 = BJet_E[0]*BJet_E[0];
                float mass = sqrt(bjetE2 - bjetP2);
                
                float dijetP2 = (Jet_Px[lq]*Jet_Px[lq] + Jet_Py[lq]*Jet_Py[lq] + Jet_Pz[lq]*Jet_Pz[lq]);
                float dijetE2 = (Jet_E[lq]*Jet_E[lq]);
                float mass2 = sqrt(dijetE2 - dijetP2);
                var       = mass1[0] + mass2 + mass;
                }
